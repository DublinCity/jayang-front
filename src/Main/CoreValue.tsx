...
const whatWeDo = [
  {
    ...
    title: "하나님을 예배해요",
    desc: [
      "삶의 예배가 회복될 때, 하나님과 사람 사이의 관계가 완전해진다고 믿어요.",
  ...
  },
  {
    ...
    title: "서로 사랑해요",
    desc: [
      `새가족, 사랑방모임, 또래모임, 각종 동아리 등등! 하나님의 사랑으로 서로를 사랑하며 즐겁게 교제해요.
  ...
  <Content>
    ...
    {whatWeDo.map(({ src, title, desc, link }, index: number) => (
      <CoreValue key={index}>
        ...
        <CoreTitle textAlign="center" fontSize="1.5rem">
          {title}
        </CoreTitle>
        ...
        {link && (
          <StyledLink to={link.url}>
            <LinkButton textAlign="center" fontSize="1.5rem" color={link.color}>
              {link.text}
            </LinkButton>
          </StyledLink>
        )}
